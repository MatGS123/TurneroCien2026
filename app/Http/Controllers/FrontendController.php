...
class FrontendController extends Controller
{
    ...
    {
        // Solo profesionales con usuario activo (paso 1 del flujo: elegir profesional)
        ...
        return view('frontend.index', compact('employees', 'obrasSociales'));
    }
    ...
    /**
     * NUEVO: Categorías donde el profesional elegido tiene al menos
     * un servicio activo cargado. Paso 2 del flujo CIEN (profesional -> categoría).
     */
    public function getEmployeeCategories(Request $request, Employee $employee)
    {
        $categories = Category::where('status', 1)
            ->whereHas('services', function ($query) use ($employee) {
                $query->where('status', 1)
        ...
        if ($categories->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'No categories available for this professional'
            ]);
        }
    ...
    /**
     * NUEVO: Servicios de una categoría, filtrados a los que ofrece
     * el profesional elegido. Paso 3 del flujo CIEN (categoría -> servicio).
     */
    public function getEmployeeServices(Request $request, Employee $employee, Category $category)
    {
        $setting = Setting::firstOrFail();
        ...
        if ($services->isEmpty()) {
            return response()->json([
                'success' => false,
                'message' => 'No services available for this professional in this category'
            ]);
        }
